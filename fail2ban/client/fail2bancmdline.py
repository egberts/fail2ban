...
class Fail2banCmdLine(object):
    ...
    def dispUsage(self):
        ...
        output(
            "    --pname <NAME>          name of the process (main thread)"
            "to identify instance (default fail2ban-server)")
        ...
        output(
            ...
            "read configuration)")
        output("    --timeout               timeout to wait for the server "
               "(for internal usage only, don't read configuration)")
